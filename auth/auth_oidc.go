...
func (o *oidcAuth) login(w http.ResponseWriter, token *oauth2.Token) (*loginState, error) {
	...
	cookie := http.Cookie{Name: openshiftSessionCookieName, Value: ls.sessionToken, MaxAge: maxAge(ls.exp, time.Now()), HttpOnly: true, Path: o.cookiePath, Secure: o.secureCookies}
