...
func TestNewLoginState(t *testing.T) {
	...
	tests := []struct {
		...
		wantID		string
		wantExp		int64
	}{{encoded: "rando-token-string", claims: fmt.Sprintf(`{
				"sub": "user-id",
				"email": "penny@example.com",
				"exp": %d
			}`, exp), wantErr: false, wantEmail: "penny@example.com", wantID: "user-id", wantExp: exp}, {encoded: "rando-token-string", claims: fmt.Sprintf(`{
				"email": "penny@example.com",
				"exp": %d
			}`, time.Now().Unix()), wantErr: true}}
