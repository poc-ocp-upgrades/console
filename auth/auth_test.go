...
type mockOpenShiftProvider struct{ issuer string }
...
type mockOIDCProvider struct{ issuer string }
...
func TestNewAuthenticator(t *testing.T) {
	...
	defer s.Close()
	p.issuer = s.URL
	ccfg := &Config{ClientID: "fake-client-id", ClientSecret: "fake-secret", Scope: []string{"foo", "bar"}, RedirectURL: "http://example.com/callback", IssuerURL: p.issuer, ErrorURL: errURL, SuccessURL: sucURL, CookiePath: "/", RefererPath: "http://auth.example.com/", SecureCookies: true}
...
func TestNewOpenShiftAuthenticator(t *testing.T) {
	...
	defer s.Close()
	p.issuer = s.URL
	ccfg := &Config{AuthSource: AuthSourceOpenShift, ClientID: "fake-client-id", ClientSecret: "fake-secret", Scope: []string{"foo", "bar"}, RedirectURL: "http://example.com/callback", IssuerURL: p.issuer, ErrorURL: errURL, SuccessURL: sucURL, CookiePath: "/", RefererPath: "http://auth.example.com/", SecureCookies: true}
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	a, err := NewAuthenticator(ctx, ccfg)
	if err != nil {
...
func TestRedirectAuthError(t *testing.T) {
	...
	ccfg := &Config{ClientID: "fake-client-id", ClientSecret: "fake-secret", RedirectURL: "http://example.com/callback", IssuerURL: "http://auth.example.com", ErrorURL: errURL, SuccessURL: sucURL, CookiePath: "/", RefererPath: "http://auth.example.com/", SecureCookies: true}
...
func testCSRF(t *testing.T, token string, cookie string, accept bool) {
	...
	if len(cookie) > 0 {
		...
		r.AddCookie(&http.Cookie{Name: CSRFCookieName, Value: cookie, MaxAge: 1000000, HttpOnly: true, Path: a.cookiePath})
	}
