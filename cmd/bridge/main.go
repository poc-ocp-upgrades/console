...
func main() {
	...
	srv := &server.Server{PublicDir: *fPublicDir, TectonicVersion: *fTectonicVersion, BaseURL: baseURL, LogoutRedirect: logoutRedirect, TectonicCACertFile: caCertFilePath, Branding: branding, DocumentationBaseURL: documentationBaseURL, GoogleTagManagerID: *fGoogleTagManagerID, LoadTestFactor: *fLoadTestFactor}
	...
	case "in-cluster":
		...
		srv.K8sProxyConfig = &proxy.Config{TLSClientConfig: tlsConfig, HeaderBlacklist: []string{"Cookie", "X-CSRFToken"}, Endpoint: k8sEndpoint}
		...
		if *fServiceCAFile != "" {
			...
			srv.PrometheusProxyConfig = &proxy.Config{TLSClientConfig: monitoringProxyTLSConfig, HeaderBlacklist: []string{"Cookie", "X-CSRFToken"}, Endpoint: &url.URL{Scheme: "https", Host: openshiftPrometheusHost, Path: "/api"}}
			srv.PrometheusTenancyProxyConfig = &proxy.Config{TLSClientConfig: monitoringProxyTLSConfig, HeaderBlacklist: []string{"Cookie", "X-CSRFToken"}, Endpoint: &url.URL{Scheme: "https", Host: openshiftPrometheusTenancyHost, Path: "/api"}}
			srv.AlertManagerProxyConfig = &proxy.Config{TLSClientConfig: monitoringProxyTLSConfig, HeaderBlacklist: []string{"Cookie", "X-CSRFToken"}, Endpoint: &url.URL{Scheme: "https", Host: openshiftAlertManagerHost, Path: "/api"}}
		}
	case "off-cluster":
		k8sEndpoint = validateFlagIsURL("k8s-mode-off-cluster-endpoint", *fK8sModeOffClusterEndpoint)
		srv.K8sProxyConfig = &proxy.Config{TLSClientConfig: &tls.Config{InsecureSkipVerify: *fK8sModeOffClusterSkipVerifyTLS}, HeaderBlacklist: []string{"Cookie", "X-CSRFToken"}, Endpoint: k8sEndpoint}
	default:
	...
	srv.K8sClient = &http.Client{Transport: &http.Transport{TLSClientConfig: srv.K8sProxyConfig.TLSClientConfig}}
	switch *fUserAuth {
	case "oidc", "openshift":
		...
		oidcClientConfig := &auth.Config{AuthSource: authSource, IssuerURL: userAuthOIDCIssuerURL.String(), IssuerCA: *fUserAuthOIDCCAFile, ClientID: *fUserAuthOIDCClientID, ClientSecret: oidcClientSecret, RedirectURL: proxy.SingleJoiningSlash(srv.BaseURL.String(), server.AuthLoginCallbackEndpoint), Scope: scopes, K8sCA: caCertFilePath, ErrorURL: authLoginErrorEndpoint, SuccessURL: authLoginSuccessEndpoint, CookiePath: cookiePath, RefererPath: refererPath, SecureCookies: secureCookies}
		if *fKubectlClientID != "" {
			srv.KubectlClientID = *fKubectlClientID
			oidcClientConfig.Scope = append(oidcClientConfig.Scope, "audience:server:client_id:"+*fUserAuthOIDCClientID, "audience:server:client_id:"+*fKubectlClientID)
		}
	...
	case "service-account":
		...
		srv.StaticUser = &auth.User{Token: k8sAuthServiceAccountBearerToken}
	case "bearer-token":
		validateFlagNotEmpty("k8s-auth-bearer-token", *fK8sAuthBearerToken)
		srv.StaticUser = &auth.User{Token: *fK8sAuthBearerToken}
	case "oidc", "openshift":
	...
	httpsrv := &http.Server{Addr: listenURL.Host, Handler: srv.HTTPHandler()}
